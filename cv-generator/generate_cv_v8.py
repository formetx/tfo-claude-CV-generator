...
def build_letter_pdf(letter: LetterData, output_path: str) -> str:
    """Génère une lettre de motivation PDF au style cohérent avec le CV."""
    ...
    # Styles lettre
    L_NAME  = ParagraphStyle('L_NAME',  fontName='Helvetica-Bold',    fontSize=16, textColor=DARK_BLUE, leading=20, spaceAfter=2)
    L_TAG   = ParagraphStyle('L_TAG',   fontName='Helvetica',          fontSize=8,  textColor=MID_BLUE,  leading=11, spaceAfter=14)
    L_META  = ParagraphStyle('L_META',  fontName='Helvetica',          fontSize=8,  textColor=MID_GREY,  leading=11)
    L_RECIP = ParagraphStyle('L_RECIP', fontName='Helvetica',          fontSize=8.5,textColor=TEXT,      leading=12)
    L_DATE  = ParagraphStyle('L_DATE',  fontName='Helvetica',          fontSize=8.5,textColor=MID_GREY,  leading=12, spaceAfter=10)
    L_SUBJ  = ParagraphStyle('L_SUBJ',  fontName='Helvetica-Bold',    fontSize=9,  textColor=DARK_BLUE, leading=13, spaceBefore=10, spaceAfter=10)
    L_BODY  = ParagraphStyle('L_BODY',  fontName='Helvetica',          fontSize=9,  textColor=TEXT,      leading=13.5, spaceAfter=8)
    L_CLOSE = ParagraphStyle('L_CLOSE', fontName='Helvetica',          fontSize=9,  textColor=TEXT,      leading=13, spaceBefore=14)
    L_SIG   = ParagraphStyle('L_SIG',   fontName='Helvetica-Bold',    fontSize=9,  textColor=DARK_BLUE, leading=13, spaceBefore=22)
    ...
    def draw_letter_page(canvas, doc):
        ...
        # Header band
        canvas.setFillColor(DARK_BLUE)
        canvas.rect(0, H - 22*mm, W, 19.5*mm, fill=1, stroke=0)
        # Nom dans le header
        canvas.setFont('Helvetica-Bold', 15)
        canvas.setFillColor(WHITE)
        canvas.drawString(LM, H - 15*mm, 'Thierry Formentini')
        # Coordonnées à droite dans le header
        canvas.setFont('Helvetica', 7)
        canvas.setFillColor(HexColor('#BDD0E8'))
        canvas.drawRightString(W - RM, H - 10*mm, 'tfo@syscon.fr  \u00b7  +49 170 44 81 81 4  \u00b7  Lante 60, D-42281 Wuppertal')
        canvas.drawRightString(W - RM, H - 15.5*mm, '10.12.1970  \u00b7  Nationality: French')
        # Footer
        canvas.setStrokeColor(HexColor('#CDD8E8'))
        canvas.setLineWidth(0.4)
        ...
        canvas.setFont('Helvetica', 6.5)
        canvas.setFillColor(MID_GREY)
        canvas.drawString(LM, 8*mm, 'Thierry Formentini  \u00b7  tfo@syscon.fr  \u00b7  +49 170 44 81 81 4')
        canvas.restoreState()
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=26*mm, bottomMargin=BM,
    )
    ...
    # Ligne séparatrice
    story.append(HRFlowable(width='100%', thickness=0.5, color=HexColor('#CDD8E8'), spaceAfter=6, spaceBefore=2))
    # Objet
    story.append(Paragraph(letter.subject, L_SUBJ))
    # Salutation
    story.append(Paragraph('Sehr geehrte Damen und Herren,', L_BODY))
    # Corps
    for para in letter.body_paragraphs:
        story.append(Paragraph(para, L_BODY))
    # Closing
    story.append(Paragraph('Ich freue mich \xfcber die M\xf6glichkeit eines pers\xf6nlichen Gespr\xe4chs.', L_CLOSE))
    story.append(Spacer(1, 3*mm))
    story.append(Paragraph('Mit freundlichen Gr\xfc\xdfen', L_BODY))
...
# ══════════════════════════════════════════════════════════════
# VARIANTE PERSONIO
# Override uniquement sur postes clés — le reste hérite du BASE_CV
# ══════════════════════════════════════════════════════════════

PERSONIO_CV = _make_variant(
    job_key='PERSONIO',
    tagline='Head of AI & Digital Transformation  \u00b7  AI Platform Owner  \u00b7  LLM in Production',

    profile=[
        'AI leader, 3+ ans livrables',
        'LLM/RAG/agents en production.',
        'Platform ownership end-to-end :',
        'gouvernance, RBAC, vendor mgmt.',
        'Change management multi-sites',
        'sans autorit\xe9 hi\xe9rarchique directe.',
    ],

    expertise=[
        'AI strategy & roadmap (Board level)',
        'LLM / RAG / Agentic workflows (prod)',
        'AI platform ownership & governance',
        'AI security & data classification',
        'Hub-and-spoke AI adoption',
        'Change management & enablement',
        'Claude Enterprise / Cowork / Claude Code',
    ],

    competencies=[
        'AI Platform Ownership (RBAC, spend)',
        'LLM / RAG deployment in production',
        'AI Governance & Security',
        'Change Management & Adoption',
        'Team Leadership & Hiring',
        'Stakeholder Management (C-level)',
        'Python, JS, Claude Code, GitHub',
        'Cloud & Cybersecurity (NIS2)',
    ],

    job_overrides={
        # Radprax — enrichi AI platform + hub-and-spoke + Claude
        0: JobEntry(
            company='RADPRAX RADIOLOGY GROUP \u2013 Wuppertal  |  30 Kliniken NRW',
            title='Head of IT & AI Transformation',
            date='03/2022 \u2013 heute',
            sections=[
                _S('AI Platform Ownership & Governance', [
                    _B('Plateforme AI interne owned end-to-end : s\xe9lection mod\xe8les, RBAC, spend management, relations fournisseurs.', 1),
                    _B('Framework s\xe9curit\xe9 AI d\xe9fini : classification donn\xe9es cliniques, r\xe8gles strictes sur ce qui peut/ne peut pas \xeatre pass\xe9 aux LLMs en production.', 1),
                    _B('Strat\xe9gie AI d\xe9finie et ex\xe9cut\xe9e ; advisor Board sur risques cyber & digitaux.', 1),
                    _B('Processus AI gouvern\xe9s d\xe9ploy\xe9s dans 30 cliniques \u2014 mod\xe8le hub-and-spoke sans autorit\xe9 hi\xe9rarchique directe.', 1),
                    _B('Architecture donn\xe9es m\xe9dicales con\xe7ue pour supporter les workloads AI en production.', 2),
                    _B('Utilisateur intensif Claude Enterprise, Cowork et Claude Code en contexte professionnel.', 2),
                ]),
                _S('AI Delivery en Production', [
                    _B('Solutions d\u2019imagerie m\xe9dicale AI d\xe9ploy\xe9es : r\xe9duction temps de scan de 50 %.', 1),
                    _B('Automatisation de 60 % des workflows diagnostics via analyse AI.', 1),
                    _B('Knowledge assistant RAG-based d\xe9ploy\xe9 sur les SOPs cliniques pour le personnel de 30 cliniques.', 1),
                    _B('Agents r\xe9ponse LLM op\xe9rationnels : helpdesk IT et customer care en production.', 2),
                ]),
                _S('IT Management', [
                    _B('Migration cloud (Ionos, Equinix) en environnement 0-trust, conformit\xe9 m\xe9dicale.', 2),
                    _B('Politiques cybers\xe9curit\xe9 globales con\xe7ues ; conformit\xe9 NIS2 impl\xe9ment\xe9e.', 2),
                    _B('Application cloud OTRS : centralisation des tests de conformit\xe9 sur 200 postes r\xe9gul\xe9s.', 3),
                    _B('PMO et gouvernance ITIL \xe9tablis : incidents, helpdesk, gestion de projets.', 3),
                ]),
            ],
        ),
        # Blue Angel — title update seulement
        2: JobEntry(
            company='BLUE ANGEL \u2013 Frankfurt',
            title='Fondateur / AI Product Builder',
            date='12/2017 \u2013 01/2021',
            sections=[
                _S(None, [
                    _B('Technologie wearable AI d\xe9velopp\xe9e pour la pr\xe9diction d\u2019urgences cardiaques.', 1),
        ...
        # AbbVie — highlight change mgmt multi-pays
        3: JobEntry(
            company='ABBVIE LIFE SCIENCE LABORATORIES \u2013 Frankfurt',
            title='Director Commercial IT Innovation \u2013 Board Member',
            date='01/2012 \u2013 03/2015',
            sections=[
                _S('Member of the Board', [
                    _B('Membre du Board Commercial allemand et du Board IT europ\xe9en (WE, Canada, EMEA).', 1),
                    _B('Liaison IT-Zentrale Chicago ; strat\xe9gie IT Westeuropa, Canada, EMEA.', 2),
                ]),
                _S('S\xe9paration Abbott \u2192 AbbVie', [
                    _B('Scission IT supervis\xe9e : infrastructure physique jusqu\u2019aux applications utilisateurs.', 1),
                    _B('IT-Personnel, Service-Portfolios, Helpdesk et End-User-Computing reorganis\xe9s.', 2),
                    _B('Transformation culturelle conduite pour \xe9tablir AbbVie comme entit\xe9 autonome.', 2),
                ]),
                _S('Programme Omnicanal Europ\xe9en', [
                    _B('CRM/CLM Veeva Salesforce d\xe9ploy\xe9 sur 24 pays en 2 ans, 800 commerciaux.', 1),
                    _B('iPads introduits pour 800 repr\xe9sentants commerciaux (CLM, CRM, documents s\xe9curis\xe9s).', 2),
    ...
    # Radprax, Blue Angel, Nephrocare, AbbVie, Jenoptik, Techem, McKinsey, Colt
    job_order=[0, 2, 1, 3, 4, 5, 6, 7],
)

PERSONIO_LETTER = LetterData(
    recipient_name='Personio',
    recipient_addr=[
        'Personio SE & Co. KG',
        'Recruiting Team',
        'M\xfcnchen / Berlin',
    ],
    city_date='Wuppertal, 2. Mai 2026',
    subject='Bewerbung als Head of Internal AI & Automation',
    body_paragraphs=[
        'Personio stellt gerade eine Funktion auf, die ich bei Radprax seit drei Jahren operativ ausf\xfclle \u2014 mit dem Unterschied, dass ich sie dort ohne Vorlage, ohne Team und ohne bestehende Plattform aufgebaut habe. Das ist der Kontext, aus dem ich mich bewerbe.',
        'Bei Radprax verantworte ich die KI-Plattform end-to-end: Modellauswahl, RBAC, Spend Management, Vendor-Beziehungen. Ich habe ein AI-Governance-Framework f\xfcr klinische Daten definiert \u2014 klare Regeln, was in den LLM-Kontext darf und was nicht. In Produktion laufen ein RAG-basierter Wissensassistent auf klinischen SOPs, LLM-Agenten f\xfcr Helpdesk und Customer Care sowie KI-gest\xfctzte Bildanalyse mit 50\u202f% k\xfcrzerer Scanzeit und 60\u202f% automatisierten Diagnose-Workflows \u2014 alles \xfcber 30 Kliniken ausgerollt, ohne direkte Weisungsbefugnis vor Ort. Claude Enterprise und Cowork nutze ich intensiv im eigenen Arbeitsalltag.',
        'Was Personio sucht \u2014 jemanden, der die Strategie setzt, die Plattform governet, ein Team aufbaut und gleichzeitig selbst liefert \u2014 das ist mein t\xe4gliches Setup. Nicht als Konzept. Als gelebte Praxis.',
    ],
)
...
AMPHENOL_CV = _make_variant(
    job_key='AMPHENOL',
    tagline='Head of Digitalization & AI  \u00b7  Hands-on AI Builder  \u00b7  Mittelstand Leadership',

    profile=[
        'AI builder & leader, 3+ ans',
        'livraisons AI en production.',
        'Prototypage Python, RAG, agents,',
        'LLMs \u2014 de z\xe9ro \xe0 prod.',
        'Exp. industrie & Mittelstand ;',
        'founder mindset + ROI chiffr\xe9.',
    ],

    expertise=[
        'AI solution design & prototyping',
        'LLM / RAG / agents (production)',
        'Python \u2014 AI pipelines & automation',
        'APIs & data integration',
        'KI-Strategie & Roadmap',
        'Prozessautomatisierung (60\u202f%+)',
        'AI governance & security',
    ],

    competencies=[
        'Hands-on AI Prototyping (Python)',
        'LLM / RAG deployment in production',
        'Digitalisierungsstrategie',
        'Cloud & APIs (Ionos, Equinix)',
        'Team Leadership & Aufbau',
        'Business Case for AI (ROI)',
        'IT Governance & Cybersecurity',
        'Founder / Entrepreneur Mindset',
    ],

    job_overrides={
        # Radprax — focus hands-on, Python, KPIs, build-from-scratch
        0: JobEntry(
            company='RADPRAX RADIOLOGY GROUP \u2013 Wuppertal  |  30 Kliniken NRW',
            title='Head of Digitalization & AI',
            date='03/2022 \u2013 heute',
            sections=[
                _S('AI Strategy & Hands-on Delivery', [
                    _B('Strat\xe9gie KI d\xe9finie et ex\xe9cut\xe9e : prototypage Python, d\xe9ploiement LLMs et pipelines RAG en production.', 1),
                    _B('Solutions d\u2019imagerie m\xe9dicale AI d\xe9ploy\xe9es : r\xe9duction temps de scan de 50 %.', 1),
                    _B('Automatisation de 60 % des workflows diagnostics via analyse AI \u2014 KPIs mesur\xe9s et track\xe9s.', 1),
                    _B('Knowledge assistant RAG-based d\xe9ploy\xe9 sur SOPs cliniques ; agents LLM helpdesk & customer care op\xe9rationnels.', 1),
                    _B('Processus AI int\xe9gr\xe9s dans 30 cliniques \u2014 build-from-scratch, sans \xe9quipe pr\xe9existante.', 2),
                    _B('Architecture donn\xe9es m\xe9dicales con\xe7ue pour supporter les workloads AI en production.', 2),
                ]),
                _S('IT & Cloud Management', [
                    _B('Migration cloud (Ionos, Equinix) en environnement 0-trust ; APIs et int\xe9grations data de bout en bout.', 1),
                    _B('Politiques cybers\xe9curit\xe9 globales con\xe7ues ; conformit\xe9 NIS2 impl\xe9ment\xe9e.', 2),
                    _B('Application cloud OTRS : centralisation des tests de conformit\xe9 sur 200 postes r\xe9gul\xe9s.', 3),
                    _B('Roadmap IT strat\xe9gique 3 ans ; PMO et gouvernance ITIL \xe9tablis.', 3),
                ]),
            ],
        ),
        # Blue Angel — founder/builder mindset
        2: JobEntry(
            company='BLUE ANGEL \u2013 Frankfurt',
            title='Gr\xfcnder / AI Product Builder',
            date='12/2017 \u2013 01/2021',
            sections=[
                _S(None, [
                    _B('Technologie wearable AI d\xe9velopp\xe9e de z\xe9ro : analyse de 6 signes vitaux, pr\xe9diction urgences cardiaques, monitoring 24/7.', 1),
                    _B('Syst\xe8me cloud con\xe7u et impl\xe9ment\xe9 en autonomie compl\xe8te.', 2),
                    _B('Prototype vendu \xe0 une multinationale pharma \u2014 German Innovation Award.', 1),
                ]),
            ],
        ),
        # Jenoptik — industrie défense/semiconductor mis en avant
        4: JobEntry(
            company='JENOPTIK OPTICAL SYSTEMS \u2013 Jena  |  D\xe9fense, Semiconductor, Optronique  |  TecDAX',
            title='CEO Shared Service Center \u2013 Corporate CTO',
            date='01/2007 \u2013 02/2011  |  Budget : 14 M\u20ac  |  \u00c9quipe : 70+',
            sections=[
                _S(None, [
                    _B('SSC cr\xe9\xe9 comme entit\xe9 juridique : IT, Finance, Achats, RH, Immobilier \u2014 70+ collaborateurs.', 1),
                    _B('\xc9conomies jusqu\u2019\xe0 80 % via optimisation sourcing (achats, IT, RH, immobilier).', 1),
                    _B('Transformation IT globale sur 5 divisions mondiales (d\xe9fense, semiconductor, m\xe9trologie).', 1),
                    _B('Migration SAP R/3, s\xe9curisation WAN, VoIP global, data centers HA (Citrix/VMware).', 1),
                    _B('Framework cybers\xe9curit\xe9 aux normes cryptographiques militaires.', 2),
                    _B('Due diligence men\xe9e pour acquisitions dans le secteur d\xe9fense.', 2),
                    _B('Centralisation des services distribu\xe9s : r\xe9duction des co\xfbts de 50 %.', 3),
    ...
    # Radprax, Blue Angel (builder), Jenoptik (industrie), Nephrocare, AbbVie, Techem, McKinsey, Colt
    job_order=[0, 2, 4, 1, 3, 5, 6, 7],
)

AMPHENOL_LETTER = LetterData(
    recipient_name='Amphenol-Air LB GmbH',
    recipient_addr=[
    ...
    city_date='Wuppertal, 2. Mai 2026',
    subject='Bewerbung als Head of Digitalization & AI (m/w/d)',
    body_paragraphs=[
        'KI-L\xf6sungen, die ich selbst in Python prototypiert, in der Cloud deployt und im Betrieb gehalten habe \u2014 das ist mein Ausgangspunkt f\xfcr diese Bewerbung.',
        'Bei Radprax habe ich eine KI-Strategie von Null aufgebaut und umgesetzt: RAG-basierter Wissensassistent auf klinischen SOPs, LLM-Agenten f\xfcr Helpdesk und Customer Care, KI-gest\xfctzte Bildanalyse mit 50\u202f% k\xfcrzerer Scanzeit und 60\u202f% automatisierten Diagnose-Workflows \u2014 alles selbst entwickelt, deployt und auf 30 Kliniken ausgerollt. Davor habe ich bei Jenoptik (TecDAX, Defense & Semiconductor) eine globale IT-Transformation \xfcber f\xfcnf Divisionen gef\xfchrt und als Gr\xfcnder von Blue Angel eine KI-Wearable-Technologie zur Herzinfarkt-Pr\xe4vention entwickelt \u2014 Prototyp, Cloud-System, Verkauf an einen internationalen Pharmakonzern.',
        'Was Amphenol-Air LB beschreibt \u2014 eigenst\xe4ndige Umsetzung konkreter Use Cases, Aufbau einer KI-Architektur, schrittweiser Teamaufbau, direkte Zusammenarbeit mit der Gesch\xe4ftsf\xfchrung \u2014 das entspricht genau der Art, wie ich gearbeitet habe. Kein Konzept ohne Umsetzung.',
    ],
)
...
VARIANTS: dict[str, CVData] = {
    'BASE':     BASE_CV,
    'HASCO':    HASCO_CV,
    'PERSONIO': PERSONIO_CV,
    'AMPHENOL': AMPHENOL_CV,
}

LETTERS: dict[str, LetterData] = {
    'PERSONIO': PERSONIO_LETTER,
    'AMPHENOL': AMPHENOL_LETTER,
}
...
def main():
    parser = argparse.ArgumentParser(description='Génère le CV + lettre de Thierry Formentini.')
    parser.add_argument('--job',    default='BASE', choices=list(VARIANTS.keys()),
                        help='Variante de CV à générer (défaut: BASE)')
    parser.add_argument('--output', default=None,
                        help='Chemin de sortie du PDF CV')
    ...
    # Lettre si disponible
    if args.job in LETTERS:
        letter_path = args.output.replace('_CV_', '_Lettre_').replace('Formentini_CV', 'Formentini_Lettre')
        build_letter_pdf(LETTERS[args.job], letter_path)
        print(f'\u2713 Lettre PDF : {letter_path}')
